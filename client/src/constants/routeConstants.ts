export const VENDOR_ROUTS = [
  { id: 8, href: "/vendor/updateProfile", title: "Ваш профиль" },
  { id: 9, href: "/vendor/currentVendor", title: "Данные вашего юр лица" },
  {
    id: 10,
    href: "/vendor/favorite",
    title: "Избранны товары",
  },
  {
    id: 6,
    href: "/vendor/personalOrders",
    title: "Ваши заказы",
  },
  {
    id: 7,
    href: "/vendor/ordersRequset",
    title: "Ваши заявки",
  },
  { id: 1, href: "/vendor/producer", title: "Создать производителя" },
  { id: 2, href: "/vendor/product", title: "Создать продукт" },
  { id: 3, href: "/vendor/type", title: "Создать новый тип продукции" },
  { id: 4, href: "/vendor/vendorProducts", title: "Ваши товары" },
  {
    id: 5,
    href: "/vendor/addProduct",
    title: "Выбрать товары для продажи из существующих",
  },
];

export const USER_ROUTES = [
  { id: 8, href: "/user/updateProfile", title: "Ваш профиль" },
  {
    id: 10,
    href: "/user/favorite",
    title: "Избранны товары",
  },
  {
    id: 6,
    href: "/user/personalOrders",
    title: "Ваши заказы",
  },
];

export const ADMIN_ROUTES = [
  { id: 8, href: "/admin/updateProfile", title: "Ваш профиль" },
  {
    id: 10,
    href: "/admin/favorite",
    title: "Избранны товары",
  },
  {
    id: 6,
    href: "/admin/personalOrders",
    title: "Ваши заказы",
  },
  { id: 1, href: "/admin/producer", title: "Создать производителя" },
  { id: 2, href: "/admin/product", title: "Создать продукт" },
  { id: 3, href: "/admin/type", title: "Создать новый тип продукции" },
];
